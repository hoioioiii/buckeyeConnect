export interface Activity {
  ...
  export const fetchActivities = async (filters: FeedFilters = {}): Promise<FeedResponse> => {
    try {
      // Build query params
      const params = new URLSearchParams();
      
      if (filters.q) params.append('q', filters.q);
      if (filters.distance && filters.distance !== 'Any Distance') params.append('distance', filters.distance);
      if (filters.time && filters.time !== 'Any Time') params.append('time', filters.time);
      if (filters.major && filters.major !== 'All Majors') params.append('major', filters.major);
      ...
      console.log(`Fetching activities from: ${API_BASE_URL}/activities?${params.toString()}`);
